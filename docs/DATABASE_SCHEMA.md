...
- **role**: ENUM — gunakan hanya: super_admin, admin_pusat, invoice_koordinator, tiket_koordinator, visa_koordinator, role_hotel, role_bus, role_invoice_saudi, role_accounting, owner. (admin_koordinator dihapus; admin_cabang deprecated; role_visa, role_ticket, role_invoice dihapus — pakai visa_koordinator, tiket_koordinator, invoice_koordinator.)
- **branch_id**: opsional. Untuk user terikat satu cabang isi branch_id; role_hotel dan role_bus boleh NULL (scope oleh backend: wilayah atau semua cabang).
...
- **Role deprecated:** `20250219000002-migrate-deprecated-roles.js` — migrasi: `admin_cabang`/`role_visa`/`role_ticket` ke pengganti. `20250228000001-migrate-role-invoice-to-invoice-koordinator.js` — migrasi `role_invoice` → `invoice_koordinator`. role_visa, role_ticket, role_invoice sudah dihapus dari aplikasi.
