...
const SuperAdminDashboard: React.FC = () => {
  ...
  const quickActions = [
    { label: 'Status User', path: '/dashboard/super-admin/users-status', icon: <Users className="w-6 h-6" />, color: 'from-emerald-500 to-teal-600' },
      ...
      <Card className="travel-card">
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <StatCard icon={<Activity className="w-5 h-5" />} label="Kesehatan Sistem" value={loading && !data ? '–' : (perf.database === 'ok' ? 'OK' : 'Error')} subtitle={loading && !data ? '–' : `Uptime: ${perf.uptime_human || '-'}`} iconClassName={perf.database === 'ok' ? 'bg-[#0D1A63] text-white' : 'bg-red-100 text-red-600'} onClick={() => navigate('/dashboard/super-admin/logs')} action={<div onClick={(e) => e.stopPropagation()}><Button variant="ghost" size="sm" className="gap-1 w-full justify-center" onClick={() => navigate('/dashboard/super-admin/logs')}><Eye className="w-4 h-4" /> Lihat</Button></div>} />
          </div>
