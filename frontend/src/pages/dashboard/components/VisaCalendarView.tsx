  ...
  if (visaProducts.length === 0) {
    return (
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-8 text-center text-slate-600">
        ...
        <p>Belum ada produk visa. Tambah visa di tab Daftar Visa.</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {!selectedProductId && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 p-6 text-center text-amber-800 text-sm">
          Pilih produk visa di atas untuk menampilkan kalender kuota per tanggal dan booking owner.
        </div>
      )}
      ...
      {selectedProductId && (
        <>
          ...
          <div className="flex flex-wrap gap-4 text-xs text-slate-600">
            <span>Keterangan: angka = dipesan/kuota per tanggal. Booking dihitung dari item visa dengan tanggal keberangkatan (travel_date).</span>
          </div>
        </>
      )}
    </div>
