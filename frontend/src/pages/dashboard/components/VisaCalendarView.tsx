...
import { FileText, Users, X, Plus } from 'lucide-react';
...
/** Persen kuota tersisa di bawah ini = "hampir penuh" (kuning) */
...
interface VisaCalendarViewProps {
  visaProducts: VisaProduct[];
  /** Dipanggil ketika user klik Tambah kuota (untuk beralih ke tab Daftar dll). Hanya untuk role selain owner. */
...
const VisaCalendarView: React.FC<VisaCalendarViewProps> = ({ visaProducts, onAddQuotaClick }) => {
  ...
  const selectedProduct = useMemo(() => visaProducts.find((p) => p.id === selectedProductId), [visaProducts, selectedProductId]);
  ...
  const fetchCalendar = useCallback((silent = false) => {
    ...
    productsApi
      .getVisaCalendar(selectedProductId, { from: fromStr, to: toStr })
      .then((res) => {
        if (res.data?.data) {
      ...
      .catch(() => {
        setCalendarData(null);
        if (!silent) showToast('Gagal memuat kalender visa', 'error');
      })
  ...
  if (visaProducts.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-10 text-center shadow-sm">
        ...
        <h3 className="text-lg font-semibold text-slate-800">Belum ada produk visa</h3>
        <p className="text-slate-600 mt-1">Tambah visa di tab Daftar Visa untuk melihat kalender kuota.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-end gap-4">
        <Autocomplete
          label="Pilih Produk Visa"
          value={selectedProductId}
          onChange={setSelectedProductId}
          options={visaProducts.map((p) => ({ value: p.id, label: `${p.name} (${p.code})` }))}
          emptyLabel="-- Pilih visa --"
          fullWidth={false}
      ...
      {!selectedProductId && (
        <div className="rounded-2xl border border-amber-200/80 bg-amber-50/80 p-8 text-center text-amber-800 text-sm shadow-sm">
          <p className="font-medium">Pilih produk visa di atas untuk menampilkan kalender kuota per tanggal dan booking owner.</p>
        </div>
      )}
      ...
      {selectedProductId && (
        <ProductCalendar<CalendarDayData>
          ...
          renderDayContent={({ dayIndex, isToday, data: day, openPopover, isPopoverOpen }) => {
            ...
            const statusStyles = status === 'full'
                ...
                <div className="flex items-start justify-between gap-1">
                  <span
                    className={`tabular-nums font-bold ${
                      isToday ? 'flex items-center justify-center min-w-[32px] h-8 px-2 rounded-lg bg-blue-600 text-white text-lg' : `text-xl text-slate-800 ${!day ? 'text-slate-300' : ''}`
                    }`}
                  >
                    {dayIndex}
                  </span>
                  {showAddQuota && (
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); onAddQuotaClick?.(); }}
                  ...
                  <div className={`mt-2 flex items-center gap-2 px-2 py-1.5 rounded-lg border text-[11px] font-semibold tabular-nums ${statusStyles}`}>
                    ...
                    <span>
                      ...
                      {day?.available !== undefined && (
                        <span className="font-normal opacity-90"> · {day.available} tersedia</span>
                      )}
                    </span>
                  </div>
                )}
...
export default VisaCalendarView;
