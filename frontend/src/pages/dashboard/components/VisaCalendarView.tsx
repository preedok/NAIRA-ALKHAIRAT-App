...
const VisaCalendarView: React.FC<VisaCalendarViewProps> = ({ visaProducts }) => {
  const { user } = useAuth();
  ...
  if (visaProducts.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-10 text-center shadow-sm">
        ...
        <p className="text-slate-600 mt-1">Tambah visa di tab Daftar Visa untuk memantau order per tanggal di kalender.</p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      {!selectedProductId && (
        <div className="rounded-2xl border border-sky-200/80 bg-sky-50/80 p-8 text-center text-sky-900 text-sm shadow-sm">
          <p className="font-medium">Pilih produk visa di atas untuk melihat ringkasan pemesanan per tanggal (monitoring order).</p>
        </div>
      )}
                ...
                <div className="flex items-start justify-between gap-1">
                  <span
                    className={`tabular-nums font-bold ${
                      isToday ? 'flex items-center justify-center min-w-[32px] h-8 px-2 rounded-lg bg-[#0D1A63] text-white text-lg' : `text-xl text-slate-800 ${!day ? 'text-slate-300' : ''}`
                    }`}
                  >
                    {dayIndex}
                  </span>
                ...
                {canSeeBookingDetails && day?.bookings?.length ? (
                  <button
                    ...
                    className={`mt-3 text-[11px] font-semibold flex items-center gap-1.5 rounded-lg py-1.5 px-2 w-full justify-center transition-colors ${
                      isPopoverOpen ? 'bg-[#0D1A63] text-white' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-800'
                    }`}
                  >
                    <Users className="w-3.5 h-3.5" />
                    {day.bookings.length} order
                  </button>
                ) : null}
              </>
            );
