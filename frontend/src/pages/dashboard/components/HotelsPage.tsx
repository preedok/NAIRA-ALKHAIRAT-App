...
const HotelsPage: React.FC<HotelsPageProps> = ({ embedInProducts }) => {
  ...
  return (
    <div className="space-y-5">
          ...
          renderRow={(hotel: HotelProduct) => {
            ...
            return (
              <tr key={hotel.id} className="hover:bg-slate-50 transition-colors border-b border-slate-200 last:border-b-0">
                ...
                <td className="px-4 py-4 align-top">
                  {avail === 'loading' && (
                    <div className="rounded-lg border border-amber-200 bg-amber-50/80 px-3 py-2 text-amber-700 text-sm">Memuat…</div>
                  )}
      ...
      {quantityModalHotel && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => !quantityFormSaving && !quantityFormLoading && setQuantityModalHotel(null)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50/50">
              <h2 className="text-base font-bold text-slate-900">Pengaturan Jumlah Kamar — {quantityModalHotel.name}</h2>
      ...
      {/* Modal Data per Musim (admin pusat): musim + inventori kamar per tanggal → availability realtime */}
      {seasonsModalHotel && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => !seasonSaving && !inventorySaving && setSeasonsModalHotel(null)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-slate-200 bg-slate-50/50">
              <h2 className="text-base font-bold text-slate-900">Data per Musim — {seasonsModalHotel.name}</h2>
            ...
            <div className="p-4 overflow-y-auto flex-1">
              ...
              {inventoryForSeason ? (
                <div>
                  <button type="button" onClick={() => setInventoryForSeason(null)} className="text-sm text-emerald-600 hover:underline mb-2">← Kembali ke daftar musim</button>
                      ...
                      <tbody>
                        {seasonsList.map((s) => (
                          <tr key={s.id} className="border-b border-slate-100">
                            ...
                            <td className="py-2 px-3 text-slate-600">{s.start_date} — {s.end_date}</td>
