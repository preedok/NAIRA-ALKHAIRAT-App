...
/** Rekap Hotel - modul standalone (kolom sama dengan spreadsheet HOTEL MEKKAH/MADINAH) */
export interface RekapHotelRecord {
  id: string;
  source_type: 'order_list' | 'allotment' | 'period_list';
  period_name?: string | null;
  season_year?: string | null;
  sort_order?: number | null;
  tentative?: string | null;   // TNTV
  definite?: string | null;    // DFNT
  client?: string | null;
  paket?: string | null;
  hotel_makkah?: string | null;  // HOTEL MEKKAH
  hotel_madinah?: string | null; // HOTEL MADINAH
  check_in?: string | null;   // IN
  check_out?: string | null;   // OUT
  total_hari?: number | null;  // TOTAL HARI
  room_d?: number | null;
  room_t?: number | null;
  room_q?: number | null;
  room_qn?: number | null;
  ...
  room?: number | null;
  pax?: number | null;
  meal_bb?: boolean | null;   // BB
  meal_fb?: boolean | null;   // FB
  status_available?: boolean | null;
  status_booked?: boolean | null;
  status_amend?: boolean | null;
  status_lunas?: boolean | null; // LUNAS
  voucher?: string | null;
  keterangan?: string | null;
  invoice_clerk?: string | null;
  ket?: string | null;
  location?: string | null;
  hotel_name?: string | null;
  room_7bed?: string | null;
  room_6bed?: string | null;
  room_quint?: string | null;
  room_quad?: string | null;
  room_triple?: string | null;
  room_double?: string | null;
  total_room?: string | null;
  status?: string | null;
  ref_number?: string | null;
  hotel_combo?: string | null;
  bandara?: string | null;
...
export const rekapHotelApi = {
  getOptions: () =>
    api.get<{ success: boolean; data: { period_names: string[]; season_years: string[]; locations: string[]; statuses: string[]; paket_types: string[]; bandaras: string[] } }>('/rekap-hotel/options'),
  list: (params?: { page?: number; limit?: number; source_type?: string; period_name?: string; season_year?: string; client?: string; location?: string; status?: string; paket_type?: string; bandara?: string; search?: string; sort_by?: string; sort_order?: 'ASC' | 'DESC' }) =>
    api.get<{ success: boolean; data: RekapHotelRecord[]; pagination: { total: number; page: number; limit: number; totalPages: number } }>('/rekap-hotel', { params }),
  getById: (id: string) => api.get<{ success: boolean; data: RekapHotelRecord }>(`/rekap-hotel/${id}`),
  create: (body: Partial<RekapHotelRecord>) => api.post<{ success: boolean; data: RekapHotelRecord }>('/rekap-hotel', body),
  update: (id: string, body: Partial<RekapHotelRecord>) => api.patch<{ success: boolean; data: RekapHotelRecord }>(`/rekap-hotel/${id}`, body),
  remove: (id: string) => api.delete<{ success: boolean; message: string }>(`/rekap-hotel/${id}`),
  bulkCreate: (items: Partial<RekapHotelRecord>[]) => api.post<{ success: boolean; data: RekapHotelRecord[]; count: number }>('/rekap-hotel/bulk', { items })
};
