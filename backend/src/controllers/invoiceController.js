...
async function loadInvoiceListRelations(data) {
  ...
  if (orderIdsFromRows.length > 0) {
    const items = await OrderItem.findAll({
      ...
      attributes: ['id', 'order_id', 'type', 'quantity', 'product_ref_id', 'unit_price', 'unit_price_currency', 'subtotal', 'manifest_file_url', 'meta', 'jamaah_data_type', 'jamaah_data_value']
    });
...
/** Satu baris per item (sama pengelompokan dengan PDF): teks harga per item & subtotal untuk Excel. */
function buildOrderItemExportPriceTexts(inv) {
  const tot = invoiceListAmountTriple(inv);
  const items = buildPdfOrderItemsDetailed(inv);
  if (!items.length) return { unitBlock: '-', subtotalBlock: '-' };
  const unitLines = [];
  const subLines = [];
  for (const it of items) {
    ...
    subLines.push(
      `Rp ${Math.round(subTriple.idr).toLocaleString('id-ID')} · SAR ${subTriple.sar.toFixed(2)} · USD ${subTriple.usd.toFixed(2)}`
    );
  }
  return { unitBlock: unitLines.join('\n'), subtotalBlock: subLines.join('\n') };
}
...
function formatDateOnlyShortForExport(value) {
  if (!value) return '-';
  ...
  return formatDateOnlyForExport(value);
}
...
function buildPdfOrderItemsDetailed(inv) {
  ...
  for (const it of src) {
    ...
    const stayLabel = (checkIn || checkOut)
      ? `CI ${formatDateOnlyShortForExport(checkIn)} | CO ${formatDateOnlyShortForExport(checkOut)}`
      : '-';
...
const exportListPdf = asyncHandler(async (req, res) => {
  ...
  if (!data.length) {
    ...
    data.forEach((inv, idx) => {
      ...
      const unitColW = Math.max(52, Math.floor(itemW * 0.27));
      const subtotalColW = Math.max(52, itemW - subItemW - unitColW);
      const xUnit = itemX + subItemW;
      const xSubtotal = xUnit + unitColW;
      ...
      doc.fillColor('#0f172a').font('Helvetica-Bold').fontSize(6.4)
        ...
        .text('Harga per item', xUnit + 2, rowTop + 3, { width: unitColW - 4, align: 'left' })
      ...
      showItems.forEach((it, i) => {
        ...
        doc.fillColor('#0f172a').fontSize(5.2).text(
          `SAR ${unitTriple.sar.toFixed(1)}\nUSD ${unitTriple.usd.toFixed(1)}`,
          xUnit + 2,
          ry + 11,
          { width: unitColW - 4, height: 14, lineGap: 0.2 }
        );
        doc.fillColor('#0f172a').font('Helvetica').fontSize(5.7).text(
          `Rp ${Math.round(subtotalTriple.idr).toLocaleString('id-ID')}`,
          xSubtotal + 2,
          ry + 2,
          { width: subtotalColW - 4, height: 10, lineBreak: false }
        );
        doc.fillColor('#0f172a').fontSize(5.2).text(
          `SAR ${subtotalTriple.sar.toFixed(1)}\nUSD ${subtotalTriple.usd.toFixed(1)}`,
          xSubtotal + 2,
          ry + 11,
          { width: subtotalColW - 4, height: 14, lineGap: 0.2 }
        );
      });
