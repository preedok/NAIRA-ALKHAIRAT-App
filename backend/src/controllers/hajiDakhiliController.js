...
const hajiDakhiliDocsDir = uploadConfig.getDir(uploadConfig.SUBDIRS.HAJI_DAKHILI_DOCS);
const hajiDakhiliUploadStorage = multer.diskStorage({
  destination: (req, file, cb) => cb(null, hajiDakhiliDocsDir),
  filename: (req, file, cb) => {
    const { dateTimeForFilename, safeExt } = uploadConfig;
    const { date, time } = dateTimeForFilename();
    const ext = safeExt(file.originalname);
    cb(null, `HAJI_DAKHILI_${req.params.orderItemId}_${date}_${time}${ext}`);
  }
});
const uploadHajiDakhiliFileMulter = multer({ storage: hajiDakhiliUploadStorage, limits: { fileSize: 25 * 1024 * 1024 } });
...
/** Sama pola bus/tiket: semua cabang aktif (role hanya super_admin & role_haji_dakhili di route). */
async function getHajiDakhiliBranchIds() {
  const branches = await Branch.findAll({ where: { is_active: true }, attributes: ['id'], raw: true });
  return branches.map((b) => b.id);
}

/**
 * GET /api/v1/haji-dakhili/dashboard
 */
const getDashboard = asyncHandler(async (req, res) => {
  ...
  const orderItemRows = await OrderItem.findAll({
    where: { type: ORDER_ITEM_TYPE.HAJI_DAKHILI },
  ...
  orderItemRows.forEach((item) => {
    ...
    totalHajiDakhiliItems += 1;
    const status = (item.meta && item.meta.haji_dakhili_status) || HAJI_DAKHILI_PROGRESS_STATUS.PENDING;
    const norm = status === HAJI_DAKHILI_PROGRESS_STATUS.COMPLETED ? 'completed' : status === HAJI_DAKHILI_PROGRESS_STATUS.IN_PROGRESS ? 'in_progress' : 'pending';
    ...
    if (norm !== 'completed') {
      pendingList.push({
        ...
        product_name: item.Product?.name || 'Haji Dakhili',
  ...
  res.json({
    ...
    data: {
      ...
      total_haji_dakhili_items: totalHajiDakhiliItems,
      by_status: byStatus,
...
/**
 * GET /api/v1/haji-dakhili/invoices
 * Daftar invoice yang order-nya punya item Haji Dakhili (sama pola tiket/bus untuk tabel Progress).
 */
const listInvoices = asyncHandler(async (req, res) => {
  const { status, page = 1, limit = 25 } = req.query;
  const branchIds = await getHajiDakhiliBranchIds();
  if (branchIds.length === 0) return res.status(403).json({ success: false, message: 'Tidak ada cabang aktif. Hubungi admin.' });

  const orderIdsFromHaji = await OrderItem.findAll({
    where: { type: ORDER_ITEM_TYPE.HAJI_DAKHILI },
    attributes: ['order_id'],
    raw: true
  }).then((rows) => [...new Set(rows.map((r) => r.order_id))]);

  if (orderIdsFromHaji.length === 0) {
  ...
  const ordersWithDpPaid = await Order.findAll({
    where: { id: orderIdsFromHaji, dp_payment_status: DP_PAYMENT_STATUS.PEMBAYARAN_DP },
    attributes: ['id'],
    raw: true
  ...
  const where = { order_id: { [Op.in]: orderIdsFromHaji }, branch_id: { [Op.in]: branchIds } };
...
const getInvoice = asyncHandler(async (req, res) => {
  ...
  if (!invoice.status || !allowedStatuses.includes(invoice.status)) {
    return res.status(403).json({ success: false, message: 'Invoice tidak tersedia untuk divisi Haji Dakhili.' });
  }
  const hajiItems = (invoice.Order?.OrderItems || []).filter((i) => i.type === ORDER_ITEM_TYPE.HAJI_DAKHILI);
  if (hajiItems.length === 0) return res.status(404).json({ success: false, message: 'Invoice ini tidak memiliki item Haji Dakhili' });

  const data = invoice.get ? invoice.get({ plain: true }) : invoice;
  (data?.Order?.OrderItems || []).forEach((oi) => {
    if (oi.type === ORDER_ITEM_TYPE.HAJI_DAKHILI && (oi.Product || oi.product)) {
...
const updateOrderItemProgress = asyncHandler(async (req, res) => {
  ...
  const { haji_dakhili_status } = req.body;

  const allowed = [HAJI_DAKHILI_PROGRESS_STATUS.PENDING, HAJI_DAKHILI_PROGRESS_STATUS.IN_PROGRESS, HAJI_DAKHILI_PROGRESS_STATUS.COMPLETED];
  if (!haji_dakhili_status || !allowed.includes(haji_dakhili_status)) {
    return res.status(400).json({ success: false, message: 'haji_dakhili_status harus salah satu: pending, in_progress, completed' });
  }

  const item = await OrderItem.findOne({
    where: { id: orderItemId, type: ORDER_ITEM_TYPE.HAJI_DAKHILI },
    include: [{ model: Order, as: 'Order', attributes: ['id', 'branch_id'] }]
  });
  if (!item) {
    return res.status(404).json({ success: false, message: 'Order item Haji Dakhili tidak ditemukan.' });
  }
  const branchIds = await getHajiDakhiliBranchIds();
  ...
  meta.haji_dakhili_status = haji_dakhili_status;
  await item.update({ meta });

  res.json({
    success: true,
    data: { order_item_id: item.id, haji_dakhili_status }
  });
});

/**
 * POST /api/v1/haji-dakhili/order-items/:orderItemId/haji-dakhili-document
 * Upload dokumen Haji Dakhili (PDF/ZIP/dll.) hanya jika status progress = selesai (completed).
 */
const uploadHajiDakhiliDocument = [
  uploadHajiDakhiliFileMulter.single('haji_dakhili_file'),
  asyncHandler(async (req, res) => {
    const { orderItemId } = req.params;
    const item = await OrderItem.findByPk(orderItemId, {
      include: [{ model: Order, as: 'Order', attributes: ['id', 'branch_id'] }]
    });
    if (!item || item.type !== ORDER_ITEM_TYPE.HAJI_DAKHILI) {
      return res.status(404).json({ success: false, message: 'Order item Haji Dakhili tidak ditemukan' });
    }
    const branchIds = await getHajiDakhiliBranchIds();
    if (!branchIds.includes(item.Order.branch_id)) {
    ...
    const st = metaNow.haji_dakhili_status || HAJI_DAKHILI_PROGRESS_STATUS.PENDING;
    if (st !== HAJI_DAKHILI_PROGRESS_STATUS.COMPLETED) {
      return res.status(400).json({
        success: false,
        message: 'Upload dokumen Haji Dakhili hanya setelah status progres "Selesai" (completed).'
      });
    }
    if (!req.file) {
    ...
    const finalName = uploadConfig.hajiDakhiliDocFilename(refLabel, item.id, req.file.originalname);
    const newPath = path.join(hajiDakhiliDocsDir, finalName);
    ...
    const fileUrl = uploadConfig.toUrlPath(uploadConfig.SUBDIRS.HAJI_DAKHILI_DOCS, savedName);
    const meta = { ...metaNow, haji_dakhili_file_url: fileUrl, haji_dakhili_file_uploaded_at: new Date().toISOString() };
    await item.update({ meta });
    res.json({
      success: true,
      data: { order_item_id: item.id, haji_dakhili_file_url: fileUrl }
    });
  })
];
